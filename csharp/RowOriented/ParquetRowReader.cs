...
namespace ParquetSharp.RowOriented
{
    ...
    public sealed class ParquetRowReader<TTuple> : IDisposable
    {
        ...
        {
            var noneMapped = Array.TrueForAll(fields, f => f.MappedSchemaName == null);
            var allMapped = Array.TrueForAll(fields, f => f.MappedSchemaName != null);
        ...
        private sealed class ExplicitColumnMapping
        {
            ...
            {
                var allUnique = fields.GroupBy(x => x.MappedSchemaName).All(g => g.Count() == 1);
                if (!allUnique)
                {
